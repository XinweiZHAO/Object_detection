...
SURF_Object_detector::SURF_Object_detector()
{
	...
	thresholdMatchingNN = 0.65;
	thresholdGoodMatches = 25;
}
SURF_Object_detector::~SURF_Object_detector()
{
	cout << "Detect Over";
...
int SURF_Object_detector::ObjimgSURF()
{
	...
	/*1. Detects keypoints in an image
	*		save the keypoints -> kpObject
	* 2. Computes the descriptors for a set of keypoints detected in an image
...
Point2f SURF_Object_detector::SURF_Objdetector(UMat capture, bool & detected_flag)
{
	...
	vector<vector<DMatch>> matches;//many matchs finded
	...
	/*==============screne information==============*/
	//captured screne frame's keypoits and descriptor
	...
	/*current screne may no detected keypoints
	*obj_descritor and screne_descritor need to be checked
	*/
	Mat img_matches;
	if (!obj_descritor.empty() && !screne_descritor.empty())
	{
		matcher.match(obj_descritor, screne_descritor, matches, 2);
		std::vector<Point2f> detected_corners;
		img_matches = drawGoodMatches(_loadobjetimage.getMat(ACCESS_READ), GRAY_screne.getMat(ACCESS_READ), obj_kpoints, \
			screne_kpoints, matches, detected_corners, screne_descritor, detected_flag);
...
%			scene_corners_: use to output the screne corners
%			screne_descritor: current screne SURF descritor
%			detected_flag: only if object is detected in current screne,
%							 the position information is meaningful
%  output para:
%			img_matches: match result
